...
- **Patch fallback policy:** Do not retry the same failed `apply_patch` approach repeatedly. If `apply_patch` fails once with format/apply errors (e.g. “No valid patches in input”, “patch does not apply”, “removal patch leaves file contents”), switch strategy immediately: re-read the target file and use `write_file` for full-file rewrites/deletes or `search_replace` for one small unique edit.
- **Retry cap:** At most 2 `apply_patch` attempts per file/change objective. If both fail, stop patch retries and continue with `write_file`/`search_replace` so progress continues within budget.
